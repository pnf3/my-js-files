document.addEventListener("DOMContentLoaded", () => {
  ...
  if (metaDescription && entryTitle.textContent.includes("Movies List")) {
    metaDescription.content = `Explore the complete ${pageTitle} movies list (${dynamicYearRange}), sorted year-wise — from debut to latest and upcoming films. Includes total number of movies to date.`;
    document.title = `${pageTitle} Movies List (Year-Wise) | Complete Filmography [${dynamicYearRange}]`;
    entryTitle.textContent = `${pageTitle} Movies List (${dynamicYearRange}) – Complete Year-Wise Filmography`;
  }
    ...
    return `
      <div class="card">
        <p style="margin-bottom:0">
          Explore the complete <span id="actor-name-1">${actorName}</span> movies list (${yearRange}), organized year-wise from debut to latest and upcoming films. This comprehensive filmography includes all movies of <span id="actor-name-2">${actorName}</span> with release years, film titles, serial numbers, and the total movie count so far.
        </p>
      </div>`;
  }
