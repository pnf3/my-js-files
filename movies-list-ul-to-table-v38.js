document.addEventListener("DOMContentLoaded", () => {
  ...
  function generateDynamicCard(actorName, yearRange) {
    return `
	<h2 id="movies-list-title">${actorName}</span> Movies List Year-Wise</h2>
      <div class="card">
        <p style="margin-bottom:0">Discover the complete, year-wise ${actorName} movies list (${yearRange}), covering films from debut to the latest and upcoming releases/last movie. This comprehensive <span id="actor-name-2">${actorName}</span> filmography includes all movies, along with release years, film titles, serial numbers, and the total number of films to date.</p>
      </div>`;
  }
  ...
  function createTable(items, actorName) {
    ...
    table.innerHTML = `
      <caption class="styled-caption">Complete List of  ${actorName} Movies by Year (${yearRange})</caption>
      <thead>
	  <tr>
 ...
 // } 
  ...
  function generateDynamicCard(actorName, yearRange) {
    return `
	<h2 id="tv-shows-list-title">${actorName}</span> TV Shows List Year-Wise</h2>
      <div class="card">
        <p style="margin-bottom:0">Explore the full, year-wise ${actorName} TV shows list (${yearRange}), from their debut series to the latest appearances. This detailed <span id="actor-name-2">${actorName}</span> showography includes release years, show titles, serial numbers, and more.</p>
      </div>`;
  }
...
function convertListToTable(list, actorName) {
  ...
  table.innerHTML = `
    <caption class="styled-caption">Full List of ${actorName} TV Shows by Year (${yearRange})</caption>
    <thead>
      <tr>
