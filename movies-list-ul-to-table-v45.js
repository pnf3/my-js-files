document.addEventListener("DOMContentLoaded", () => {
  ...
  if (metaDescription && entryTitle.textContent.includes("Movies List")) {
    ...
    document.title = `${pageTitle} All Movies List (${dynamicYearRange})`;
  ...
  function generateDynamicCard(actorName, yearRange) {
    return `<div class="card"><p class="mb-0">Explore the complete, year-wise list of ${actorName}'s movies (${yearRange}) — from the debut film to the latest release. This comprehensive filmography includes all movie titles in chronological order, with release years, a running count, and the total number of films to date.</p></div>
...
if (movieTables.length > 0) {
  ...
  function generateDynamicCard(actorName, yearRange) {
    return `
      <div class="card">
        <p class="mb-0">Explore the complete, year-wise ${actorName} TV shows list (${yearRange}), from their debut series to the latest appearances. This detailed ${actorName} showography includes release years, show titles, running count, and more.</p>
      </div>
