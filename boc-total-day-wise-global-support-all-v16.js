    ...
    tbody.querySelectorAll("tr").forEach(row => {
        ...
        if (movieCell) {
            ...
            if (dayValues[movieTitle]) {  // Exact match with data values
                // Use manually set "total" instead of summing days
                let totalGross = dayValues[movieTitle].total ?? "N/A"; 

                // Add new column with total BOC
                let bocCell = document.createElement("td");
                bocCell.textContent = totalGross; // Display manually set total
