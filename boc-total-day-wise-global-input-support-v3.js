...
// Recalculate total only if it's null or undefined
if (data.total === null || data.total === undefined) {
    data.total = data[lastDay] || 0; // Start with last known total

    // Add only future predicted differences
    for (let day = lastDay + 1; day <= finalDay; day++) {
        if (typeof data[day] === "number" && typeof data[day - 1] === "number") {
            data.total += (data[day] - data[day - 1]);
        }
    }
