...
document.addEventListener("DOMContentLoaded", function() {
    ...
    if (metaDescription) {
        if (entryTitle.textContent.includes("Box Office Collection")) {
            metaDescription.content = `${movieName} Movie Box Office Collection: Track worldwide earnings, day-wise updates, and total gross up to ${latestDay}. Stay tuned for more!`;
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // Select the movie table header and body
    const tableHead = document.querySelector(".movies-table thead tr");
    ...
    if (!document.querySelector(".movies-table thead th.release-date-header")) {
        ...
        tableHead.insertBefore(releaseDateHeader, tableHead.firstChild); // Insert at the start
    }
    ...
    if (!document.querySelector(".movies-table thead th.boc-header")) {
        ...
        tableHead.appendChild(bocHeader);
    }
