document.addEventListener("DOMContentLoaded", () => {
  ...
  if (metaDescription && entryTitle.textContent.includes("Movies List")) {
    metaDescription.content = `<meta name="description" content="Discover the complete <strong>${actorName} movies list year-wise</strong>, covering all films from ${yearRange.start} to ${yearRange.end}.">`;
  ...
  function generateDynamicCard(actorName, yearRange) {
    return `
      <div class="card">
        <p style="margin-bottom:0">Discover the complete <strong>${actorName} movies list year-wise</strong>, covering all films from ${yearRange.start} to ${yearRange.end}. This comprehensive <strong>${actorName} filmography</strong> includes every movie from debut to the latest and upcoming releases — featuring release years, film titles, serial numbers, and the total movies count to date.</p>
      </div>`;
  }
