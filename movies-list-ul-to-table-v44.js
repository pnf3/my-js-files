document.addEventListener("DOMContentLoaded", () => {
  ...
  if (metaDescription && entryTitle.textContent.includes("Movies List")) {
    metaDescription.content = `<meta name="description" content="Explore the complete, year-wise list of ${pageTitle}'s movies (${dynamicYearRange}) — from the debut film to the latest release.">`;
  ...
  function generateDynamicCard(actorName, yearRange) {
	...
	<h2 id="movies-list-title">${actorName} Movies List Year-Wise</h2>
      <div class="card">
        <p style="margin-bottom:0">Explore the complete, year-wise list of ${actorName}'s movies (${yearRange}) — from the debut film to the latest release. This comprehensive filmography includes all movie titles in chronological order, with release years, a running count, and the total number of films to date.</p>
      </div>`;
  }
...
if (movieTables.length > 0) {
  ...
  function generateDynamicCard(actorName, yearRange) {
	...
	<h2 id="tv-shows-list-title">${actorName} TV Shows List Year-Wise</h2>
      <div class="card">
        <p style="margin-bottom:0">Explore the complete, year-wise ${actorName} TV shows list (${yearRange}), from their debut series to the latest appearances. This detailed ${actorName} showography includes release years, show titles, running count, and more.</p>
      </div>`;
  }
...
if (movieTables.length > 0) {
  ...
  function generateDynamicCard(actorName, yearRange) {
    return `
	<h2 id="producer-movies-list-title">${actorName} Movies List as a Producer</h2>
      `;
  }
...
function convertListToTable(list, actorName) {
  ...
  table.innerHTML = `
    <caption class="styled-caption">Full List of ${actorName} Movies as a Producer</caption>
    <thead>
      <tr>
        <th colspan="3" scope="colgroup">${actorName} Movies List as a Producer</th>
      </tr>
      <tr>
        <th scope="col">S. No.</th>
        <th scope="col">Release Year</th>
