...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (difference > 0) {
        totalSum += difference; // ✅ Add only the difference to avoid overcounting
        if (currentWeek) {
    ...
    if (metaDescription) {
        if (entryTitle.textContent.includes("Box Office Collection")) {
            metaDescription.content = `${movieName} Box Office Collection: Track worldwide earnings, daily updates, and total gross up to ${latestDay}. Stay tuned for more!`;
