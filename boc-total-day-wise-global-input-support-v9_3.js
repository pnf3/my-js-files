...
let rating = (typeof rawRating === "number" && rawRating > 0) ? rawRating : null;

        let budget = data.Budget;
		
		if (!rating) {
        data.Verdict = "Coming Soon";
        continue;
    }
...
// Normalize both title and keys (remove case, symbols, extra words)
...
let normalizedPageTitle = normalizeTitle(pageTitle);
let matchedKey = null;

// Try to find the closest matching movie key
for (let key in window.dayValues) {
    let normalizedKey = normalizeTitle(key);
    if (normalizedPageTitle.includes(normalizedKey) || normalizedKey.includes(normalizedPageTitle)) {
        matchedKey = key;
        break;
    }
}

// If a match found, use that movie’s data
var movieData = matchedKey ? window.dayValues[matchedKey] : null;
        ...
        if (movieData) {
			...
			var crew = movieData["Crew"] || "Unknown";
          ...
          //  document.getElementById("totalSum-2").textContent = totalBoxOffice.toFixed(2);
			...
			document.getElementById("crew-2").textContent = crew;
        } else {
